...
public:
  ...
  Schema(
    SchemaDocumentType * schemaDocument, const PointerType & p, const ValueType & value,
    ...
    defaultValueLength_(0)
  {
    ...
    if (const ValueType * v = GetMember(value, GetPatternPropertiesString())) {
      ...
      for (ConstMemberIterator itr = v->MemberBegin(); itr != v->MemberEnd(); ++itr) {
        ...
        schemaDocument->CreateSchema(
          &patternProperties_[patternPropertyCount_].schema, q.Append(itr->name, allocator_),
          itr->value, document);
        patternPropertyCount_++;
      }
    }
      ...
      for (ConstMemberIterator itr = dependencies->MemberBegin(); itr != dependencies->MemberEnd();
        ...
        if (FindPropertyIndex(itr->name, &sourceIndex)) {
          if (itr->value.IsArray()) {
            ...
            schemaDocument->CreateSchema(
              &properties_[sourceIndex].dependenciesSchema, q.Append(itr->name, allocator_),
              itr->value, document);
    ...
    if (const ValueType * v = GetMember(value, GetAdditionalPropertiesString())) {
      if (v->IsBool())
        ...
        schemaDocument->CreateSchema(
          &additionalPropertiesSchema_, p.Append(GetAdditionalPropertiesString(), allocator_), *v,
          document);
    }
...
#define RAPIDJSON_STRING_(name, ...)                                                \
  ...
  RAPIDJSON_STRING_(
    PatternProperties, 'p', 'a', 't', 't', 'e', 'r', 'n', 'P', 'r', 'o', 'p', 'e', 'r', 't', 'i',
    'e', 's')
  RAPIDJSON_STRING_(
    AdditionalProperties, 'a', 'd', 'd', 'i', 't', 'i', 'o', 'n', 'a', 'l', 'P', 'r', 'o', 'p', 'e',
    'r', 't', 'i', 'e', 's')
  ...
  RAPIDJSON_STRING_(
    ExclusiveMinimum, 'e', 'x', 'c', 'l', 'u', 's', 'i', 'v', 'e', 'M', 'i', 'n', 'i', 'm', 'u',
    'm')
  RAPIDJSON_STRING_(
    ExclusiveMaximum, 'e', 'x', 'c', 'l', 'u', 's', 'i', 'v', 'e', 'M', 'a', 'x', 'i', 'm', 'u',
    'm')
  ...
  */
    ...
    schemaRef_(allocator, kInitialSchemaRefSize)
  {
    ...
    new (typeless_) SchemaType(
      this, PointerType(), ValueType(kObjectType).Move(), ValueType(kObjectType).Move(),
      allocator_);
    ...
    if (itr->value.IsString()) {
      ...
      if (len > 0) {
        ...
        if (i > 0) {  // Remote reference, resolve immediately
          if (remoteProvider_) {
            if (
              const GenericSchemaDocument * remoteDocument =
  ...
  */
  ...
  PointerType GetInvalidDocumentPointer() const
  {
    if (documentStack_.Empty()) {
      ...
      return PointerType(
        documentStack_.template Bottom<Ch>(), documentStack_.GetSize() / sizeof(Ch));
    }
  }
  ...
  {
    AddNumberError(
      SchemaType::GetMaximumString(), ValueType(actual).Move(), expected,
  ...
  {
    AddNumberError(
      SchemaType::GetMinimumString(), ValueType(actual).Move(), expected,
  ...
  {
    AddNumberError(
      SchemaType::GetMaxLengthString(), ValueType(str, length, GetStateAllocator()).Move(),
      SValue(expected).Move());
  }
  void TooShort(const Ch * str, SizeType length, SizeType expected)
  {
    AddNumberError(
      SchemaType::GetMinLengthString(), ValueType(str, length, GetStateAllocator()).Move(),
      SValue(expected).Move());
  }
  ...
  {
    AddNumberError(
      SchemaType::GetMinItemsString(), ValueType(actualCount).Move(), SValue(expectedCount).Move());
  }
  void TooManyItems(SizeType actualCount, SizeType expectedCount)
  {
    AddNumberError(
      SchemaType::GetMaxItemsString(), ValueType(actualCount).Move(), SValue(expectedCount).Move());
  }
  ...
  {
    AddNumberError(
      SchemaType::GetMaxPropertiesString(), ValueType(actualCount).Move(),
      SValue(expectedCount).Move());
  }
  void TooFewProperties(SizeType actualCount, SizeType expectedCount)
  {
    AddNumberError(
      SchemaType::GetMinPropertiesString(), ValueType(actualCount).Move(),
      SValue(expectedCount).Move());
  }
  ...
  void DisallowedProperty(const Ch * name, SizeType length)
  {
    currentError_.SetObject();
    currentError_.AddMember(
      GetDisallowedString(), ValueType(name, length, GetStateAllocator()).Move(),
      GetStateAllocator());
  ...
  void EndMissingDependentProperties(const SValue & sourceName)
  {
    if (!missingDependents_.Empty())
      currentError_.AddMember(
        ValueType(sourceName, GetStateAllocator()).Move(), missingDependents_, GetStateAllocator());
  }
...
private:
  ...
  GenericSchemaValidator(
    const SchemaDocumentType & schemaDocument, const SchemaType & root, const char * basePath,
    size_t basePathSize,
#if RAPIDJSON_SCHEMA_VERBOSE
    unsigned depth,
#endif
  ...
  void AddErrorLocation(ValueType & result, bool parent)
  {
    ...
    sb.Clear();
    memcpy(
      sb.Push(CurrentSchema().GetURI().GetStringLength()), CurrentSchema().GetURI().GetString(),
  ...
  void AddNumberError(
    const typename SchemaType::ValueType & keyword, ValueType & actual, const SValue & expected,
    ...
    SizeType count)
  {
    ...
    for (SizeType i = 0; i < count; ++i)
      errors.PushBack(
        static_cast<GenericSchemaValidator *>(subvalidators[i])->GetError(), GetStateAllocator());
    currentError_.SetObject();
