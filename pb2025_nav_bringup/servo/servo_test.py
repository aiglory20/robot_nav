...
from servo_action_group import ActionGroupManager
...
class WaypointListener(Node):
    ...
    def __init__(self, controller):
        super().__init__('servo_waypoint_listener')
        self.controller = controller

        # 订阅航点状态
        self.waypoint_sub = self.create_subscription(
            String, '/waypoint_status', self.waypoint_callback, 10
        )
        self.get_logger().info("✅ 航点监听节点已初始化，等待导航导航消息...")

    def waypoint_callback(self, msg):
        """接收航点状态回调"""
        waypoint = msg.data
        self.get_logger().info(f"📍 收到航点消息: {waypoint}")

        # 根据不同的航点执行对应的机械臂动作
        if waypoint == "A":
            self.get_logger().info("🎯 到达A点，执行比耶动作...")
            self.controller.action_group.action_3_yeah()
        elif waypoint == "B":
            self.get_logger().info("🎯 到达B点，执行挥手动作...")
            self.controller.action_group.action_2_wave()
        elif waypoint == "C":
            self.get_logger().info("🎯 到达C点，执行指示动作...")
            self.controller.action_group.action_1_indicate()
        else:
            self.get_logger().warn(f"⚠️ 未知航点: {waypoint}")

class BusServoController:
    def __init__(self):
        self.ser = None
        self.running = True
        self.action_group = ActionGroupManager(self)
    ...
    def show_help(self):
        ...
        print("     [o] 回到启动位  [e] 紧急停止 (急停)")
        print("     [1] 动作1-指示  [2] 动作2-挥手  [3] 动作3-比耶")
        print("     [i] 广播读取ID  [p] 读取位置")
        print("     [k] 释放扭力    [r] 恢复扭力")
        print("     [v] 读取温压    [q] 退出程序")
        print("=" * 43)
        ...
        while self.running:
            try:
                ...
                if cmd_lower in ['q', '0', 'quit', 'exit']:
                    ...
                    self.action_group.emergency_stop()
                elif cmd_lower == 'o':
                    self.action_group.action_reset()
                elif cmd_lower == '1':
                    self.action_group.action_1_indicate()
                elif cmd_lower == '2':
                    self.action_group.action_2_wave()
                elif cmd_lower == '3':
                    self.action_group.action_3_yeah()
                elif cmd_lower == 'i':
...
def main():
    ...
    # 创建航点监听节点
    waypoint_listener = WaypointListener(controller)
