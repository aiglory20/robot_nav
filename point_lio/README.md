...
B. Please obtain the saturation values of your used IMU (i.e., accelerator and gyroscope), and the units of the accelerator of your used IMU, then modify the .yaml file according to those settings, including values of 'satu_acc', 'satu_gyro', 'acc_norm'. That's important.
...
D. We recommend to set the **extrinsic_est_en** to false if the extrinsic is given. As for the extrinsic initialization, please refer to our recent work: [**Robust and Online LiDAR-inertial Initialization**](https://github.com/hku-mars/LiDAR_IMU_Init).
...
### 2.2 Produce high odometry output frequency and high bandwidth
...
- Since the Point-LIO supports Livox serials LiDAR, so the **livox_ros_driver** must be installed and **sourced** beforee run any Point-LIO luanch file.
...
- Remember to source the livox_ros_driver2 beforee build (follow 3.3 **livox_ros_driver**)
...
Need to setup some parameters before run:
